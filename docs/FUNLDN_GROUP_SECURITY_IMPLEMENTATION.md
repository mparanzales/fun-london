...
- `public.plan_room_create_attempts` (0005) — the same argument applied to `create_plan_room`, which was granted to `authenticated` and reachable the same way but throttled only in the server action. 10 per hour per account, `errcode 53400`.
...
| 5c | 🧨 **`0005` — APPLY AS ONE TRANSACTION, before merging the create-throttle PR** | Adds the DB-side create throttle (`plan_room_create_attempts`, 10/hour, `errcode 53400`) and **drops** `create_plan_room`'s `p_code` parameter. No client change is coupled to it — the deployed client already calls with no argument — but the nightly purge script counts the new ledger, so applying the migration first keeps `pnpm purge:plan-rooms` clean. **Half-applying the file leaves TWO signatures, and a no-argument call against both is ambiguous** (Postgres 42725 → PostgREST HTTP 300 → every room creation fails). | `EXPECT_STAGE=3 EXPECT_0004=1 pnpm verify-room-security` exits 0 and reports *"0005 applied: exactly one create_plan_room, taking no arguments"*; staging `T-2` trips at attempt 11 |
...
- **After 0001 or 0002 (nothing removed yet):** `drop policy if exists "plan room members read"/"plan room members write" on realtime.messages;` then `drop table public.plan_room_join_attempts, public.plan_room_members, public.plan_rooms cascade;` and drop the functions — **nine after 0001, TEN once 0004 has added `new_plan_room_code`**. **Four** tables once `0005` has been applied, not two — `plan_room_join_attempts` **and** `plan_room_create_attempts` both store user ids, so a rollback that forgets either leaves personal data behind. (It was "three" before 0005 added the create-side throttle ledger.) Behaviour returns to today's exactly.
